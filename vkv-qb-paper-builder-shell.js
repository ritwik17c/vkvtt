...
// Phase E5.1: sections, live scored marks, internal choice, Answer Any N groups, reordering and verified-bank insertion. No Firestore writes.
(function(){
  ...
  function load(){
    try{Object.assign(state,JSON.parse(localStorage.getItem(STORAGE)||'{}'))}catch(_){}
    state.sections=(state.sections||[]).map(s=>({...s,name:s.name||'',instructions:s.instructions||'',targetMarks:Number(s.targetMarks)||0,attemptAny:Math.max(0,Math.floor(Number(s.attemptAny)||0)),questions:(s.questions||[]).map(q=>({...q,choice:q.choice||''}))}));
  }
  ...
  function fallbackSectionScore(s){const availableMarks=(s.questions||[]).reduce((a,q)=>a+(Number(q.marks)||0),0);return{valid:true,attemptAny:0,questionCount:(s.questions||[]).length,availableMarks,countedMarks:availableMarks,problems:[]}}
  function sectionScore(s){return window.__vkvQbPaperScoring?.sectionScore?.(s)||fallbackSectionScore(s)}
  function paperScore(){
    if(window.__vkvQbPaperScoring?.paperScore)return window.__vkvQbPaperScoring.paperScore(state);
    const sections=state.sections.map(fallbackSectionScore);return{valid:true,sections,availableMarks:sections.reduce((a,s)=>a+s.availableMarks,0),countedMarks:sections.reduce((a,s)=>a+s.countedMarks,0),problems:[]};
  }
  ...
  function addVerifiedQuestion(q,sectionIndex){
    ...
    if(!state.sections.length)state.sections.push({name:'',instructions:'',targetMarks:0,attemptAny:0,questions:[]});
  ...
  function sectionStatus(s){
    const score=sectionScore(s),target=Number(s.targetMarks)||0;
    if(!score.valid)return`⚠ Answer Any rule unresolved · ${score.availableMarks} available marks`;
    const mode=score.attemptAny?`${score.countedMarks} counted · answer any ${score.attemptAny} of ${score.questionCount}`:`${score.countedMarks} marks`;
    if(!target)return mode;
    const remain=target-score.countedMarks;
    return remain===0?`${mode} / target ${target} ✓`:remain>0?`${mode} / target ${target} · ${remain} short`:`${mode} / target ${target} · ${Math.abs(remain)} over`;
  }
  function render(){
    const score=paperScore(),target=Number(state.target)||0,remain=score.valid?target-score.countedMarks:null;
    $('pbUsed').textContent=score.valid?score.countedMarks:'—';$('pbTargetView').textContent=target;$('pbRemaining').textContent=score.valid?remain:'—';$('pbRemaining').style.fontWeight='800';
    $('pbBalance').textContent=!score.valid?'Resolve the Answer Any marks rule before relying on the paper total.':!target?'Set target marks':remain===0?'✓ Paper matches target':remain>0?remain+' marks still to allocate':Math.abs(remain)+' marks over target';
    $('pbSections').innerHTML=state.sections.length?state.sections.map((s,si)=>`<div class="qcard"><div class="actions" style="justify-content:space-between;align-items:center"><div class="actions" style="gap:5px;align-items:center"><b>Section ${String.fromCharCode(65+si)}</b><button data-sup="${si}" title="Move section up" aria-label="Move section up" ${si===0?'disabled':''}>↑</button><button data-sdown="${si}" title="Move section down" aria-label="Move section down" ${si===state.sections.length-1?'disabled':''}>↓</button></div><span class="small">${esc(sectionStatus(s))}</span></div><div class="grid3" style="margin-top:8px"><div><label>Section title</label><input data-sn="${si}" value="${esc(s.name)}" placeholder="e.g. Reading / Grammar / Long Answer"></div><div><label>Instructions</label><input data-si="${si}" value="${esc(s.instructions)}" placeholder="Optional section instruction"></div><div><label>Section Target Marks</label><input data-st="${si}" type="number" min="0" step="0.5" value="${Number(s.targetMarks)||''}" placeholder="Optional"></div></div><div style="max-width:250px"><label>Answer Any (optional)</label><input data-sa="${si}" type="number" min="1" max="${Math.max(1,(s.questions||[]).length)}" step="1" value="${Number(s.attemptAny)||''}" placeholder="All questions"><div class="small">Use only when every choice carries equal marks.</div></div>${s.questions.map((q,qi)=>`<div style="display:grid;grid-template-columns:auto minmax(0,1fr) 82px auto;gap:8px;align-items:start;margin-top:10px"><b>${qi+1}.</b><div><textarea data-q="${si}:${qi}" style="min-height:58px">${esc(q.text)}</textarea>${q.sourceQuestionId?`<div class="small" style="margin-top:3px">✓ From Verified Bank</div>`:''}<textarea data-c="${si}:${qi}" style="min-height:48px;margin-top:5px" placeholder="Optional internal choice: OR ...">${esc(q.choice)}</textarea></div><input data-m="${si}:${qi}" type="number" min="0" step="0.5" value="${qmarks(q)}" aria-label="Marks"><div class="actions" style="gap:4px;flex-wrap:nowrap"><button data-up="${si}:${qi}" title="Move question up" aria-label="Move question up" ${qi===0?'disabled':''}>↑</button><button data-down="${si}:${qi}" title="Move question down" aria-label="Move question down" ${qi===s.questions.length-1?'disabled':''}>↓</button><button data-del="${si}:${qi}" title="Remove question" aria-label="Remove question">×</button></div></div>`).join('')}<div class="actions" style="margin-top:9px"><button data-addq="${si}">＋ Add Question</button><button data-dels="${si}">Remove Section</button></div></div>`).join(''):'<div class="empty">Add a section to begin building the paper.</div>';
    ...
    document.querySelectorAll('[data-sa]').forEach(e=>{e.oninput=()=>{state.sections[+e.dataset.sa].attemptAny=Math.max(0,Math.floor(Number(e.value)||0));persist()};e.onchange=()=>render()});
  ...
  function init(){
    ...
    $('pbAddSection').onclick=()=>{state.sections.push({name:'',instructions:'',targetMarks:0,attemptAny:0,questions:[{text:'',choice:'',marks:1}]});persist();render()};$('pbClear').onclick=()=>{if(confirm('Clear this local paper draft?')){localStorage.removeItem(STORAGE);location.reload()}};
    ...
    window.__vkvQbPaperBuilder={addVerifiedQuestion,getState:()=>JSON.parse(JSON.stringify(state)),calculate:paperScore,render};
