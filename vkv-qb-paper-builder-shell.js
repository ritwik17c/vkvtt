...
// Phase E2.2: editable sections + live marks + internal choice + local reordering + safe verified-bank insertion API. No Firestore writes.
(function(){
  const STORAGE='vkvtt.qb.paperDraft.v1';
  const $=id=>document.getElementById(id);
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]));
  const state={title:'',className:'',subject:'',exam:'',duration:'',target:0,sections:[]};
  function load(){try{Object.assign(state,JSON.parse(localStorage.getItem(STORAGE)||'{}'))}catch(_){} state.sections=(state.sections||[]).map(s=>({...s,name:s.name||'',instructions:s.instructions||'',questions:(s.questions||[]).map(q=>({...q,choice:q.choice||''}))}));}
  ...
  function addVerifiedQuestion(q,sectionIndex){
    ...
    if(!state.sections.length)state.sections.push({name:'',instructions:'',questions:[]});
  ...
  function init(){
    ...
    $('pbAddSection').onclick=()=>{state.sections.push({name:'',instructions:'',questions:[{text:'',choice:'',marks:1}]});persist();render()};$('pbClear').onclick=()=>{if(confirm('Clear this local paper draft?')){localStorage.removeItem(STORAGE);location.reload()}};
