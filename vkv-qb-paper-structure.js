// VKVTT QB Paper Builder — Phase E4.1 local-only structure checks
...
(function(){
 ...
 function analyse(){
  ...
  sections.forEach((s,si)=>{let st=0;const texts=[...s.querySelectorAll('[data-q]')],marks=[...s.querySelectorAll('[data-m]')],ors=[...s.querySelectorAll('[data-c]')],sectionTarget=num(s.querySelector('[data-st]')?.value);questions+=texts.length;texts.forEach(x=>{if(!x.value.trim())blank++});marks.forEach(x=>{const m=num(x.value);st+=m;used+=m;if(m<=0)zero++});ors.forEach(x=>{if(x.value.trim())choices++});sectionTotals.push({name:String.fromCharCode(65+si),marks:st,target:sectionTarget})});
