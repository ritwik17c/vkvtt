...
async function saveStatusRecord(){
 ...
 if(selectedStatus==="full"||selectedStatus==="half"){
   const cat=String(E("leaveCategory").value||"").trim().toUpperCase();if(!cat||!LEAVE_CATEGORY_LABELS[cat]){E("statusMsg").textContent="Select the leave category (VL / EL / CL / SEL / EOL / Maternity Leave).";return}
