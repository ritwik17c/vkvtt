const CACHE_NAME='vkvtt-shell-2026-09-16-admin-66-4-1';
const REMOVED_THEME_ASSET='./vkv-theme-contrast-hardening.css';
...
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE_NAME).then(cache=>cache.addAll(APP_SHELL.filter(path=>path!==REMOVED_THEME_ASSET))));self.skipWaiting();});
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE_NAME).map(k=>caches.delete(k)))));self.clients.claim();});
function injectControllers(html,url){
  const tags=[];const path=new URL(url).pathname;const isHome=/\/index\.html$/i.test(path)||/\/(?:vkvtt-preview-2|vkvtt)\/?$/i.test(path);const isAdmin=/admin-dashboard\.html$/i.test(path);
  if(isAdmin){
    html=html
      .replace("const CURRENT_APP_VERSION = '66.0';","const CURRENT_APP_VERSION = '66.4';")
      .replace(/Cloud v66\.0/g,'Cloud v66.4')
      .replace(/v66-admin-reference-polish\.js\?v=[^\"']+/g,'v66-admin-reference-polish.js?v=20260916-disabled-stable-1')
      .replace(/vkv-admin-dashboard-runtime\.js\?v=[^\"']+/g,'vkv-admin-dashboard-runtime.js?v=20260916-leave-unified-stable-1')
      .replace(/vkv-theme\.js\?v=[^\"']+/g,'vkv-theme.js?v=20260916-header-safe-1');
  }
  if(!/vkv-theme\.js/i.test(html))tags.push('<script src="./vkv-theme.js?v=20260916-header-safe-1"></script>');
  ...
  if(!tags.length)return html;const tag=tags.join('');if(/<\/head>/i.test(html))return html.replace(/<\/head>/i,tag+'</head>');if(/<body/i.test(html))return html.replace(/<body/i,tag+'<body');return tag+html;
}
function isPresentationAsset(request){try{const p=new URL(request.url).pathname;return /\/(vkv-theme\.js|vkv-theme-contrast-fixes\.css|vkv-home-header-typography\.css|vkv-home-card-cleanup\.js|vkv-annual-calendar-theme\.js|vkv-light-screen\.css|vkv-black-gold-screen\.css|vkv-campus-header\.jpg|v66-home-shell-v662\.css)$/i.test(p)}catch(_){return false}}
self.addEventListener('fetch',event=>{
  if(event.request.method!=='GET')return;
  if(event.request.mode==='navigate'){event.respondWith((async()=>{try{const response=await fetch(event.request,{cache:'no-store'}),type=response.headers.get('content-type')||'';if(response.ok&&/text\/html/i.test(type)){const html=injectControllers(await response.text(),event.request.url),headers=new Headers(response.headers);headers.set('Cache-Control','no-store');headers.delete('Content-Length');return new Response(html,{status:response.status,statusText:response.statusText,headers})}return response}catch(_){const hit=await caches.match(event.request);return hit||caches.match('./index.html')}})());return}
  if(isPresentationAsset(event.request)){event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));return response}).catch(()=>caches.match(event.request,{ignoreSearch:true})));return}
