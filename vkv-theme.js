...
(function(){
  ...
  function allThemeLinks(doc=document){
    const bg=ensureCss(doc,'vkvBlackGoldThemeCss','./vkv-black-gold-screen.css?v=20260915-theme-consistency-1');
    const lt=ensureCss(doc,'vkvLightThemeCss','./vkv-light-screen.css?v=20260915-theme-consistency-1');
  ...
  function syncEmbeddedFrames(next=theme){
    if(window.self!==window.top)return;
    document.querySelectorAll('iframe').forEach(frame=>{
      if(!frame.dataset.vkvThemeSync){
        frame.dataset.vkvThemeSync='1';
        frame.addEventListener('load',()=>syncEmbeddedFrame(frame,theme));
      }
      syncEmbeddedFrame(frame,next);
    });
  }
  function syncEmbeddedFrame(frame,next){
    try{
      const doc=frame.contentDocument;
      if(doc&&doc.documentElement)apply(next,doc,false);
    }catch(error){
      console.info('[VKVTT theme] Cross-origin frame left unchanged:',error?.message||error);
    }
  }
  ...
  function mount(){
    ...
    apply(theme);syncEmbeddedFrames(theme);registerThemeShell();
