...
function install(){
 ...
 function scanAttention(){
  ...
  const dup=text.match(/\b([A-Za-z][A-Za-z'-]{1,})\s+\1\b/i);
  const punct=text.match(/(?:\?\?|!!|,,|;;|::)/);
  if(dup)notes.push('Possible repeated word: “'+dup[1]+' '+dup[1]+'”');
 ...
 ta.addEventListener('input',()=>{if(undoValue&&ta.value!==appliedValue){undoValue='';appliedValue='';undoCorrection.style.display='none'}clearTimeout(ta.__qbAttentionTimer);ta.__qbAttentionTimer=setTimeout(scanAttention,450)});scanAttention();
