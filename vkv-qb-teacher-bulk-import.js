...
async function importAll(requestedStatus){
  ...
  try{
    for(let index=0;index<items.length;index++){
      ...
      if(coordinatorSubmit){const verifiedAt=Date.now(),verifiedWorkflow=[...workflow,{action:'subject_coordinator_self_verification',atMs:verifiedAt,byUid:user.uid,byName:actor,subject:item.subject}];try{await setDoc(ref,{status:'approved',verifiedAtMs:verifiedAt,verifiedByUid:user.uid,verifiedByEmail:mail(user.email),verifiedByName:actor,updatedAtMs:verifiedAt,workflow:verifiedWorkflow},{merge:true})}catch(error){console.warn('Question imported; coordinator self-verification was not permitted.',error)}}
