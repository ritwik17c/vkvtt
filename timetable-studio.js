...
function lessonCardHtml(record){
  const s=subjectInfo(record.subjectId),selected=state.selectedRecordId===record.id,filter=$('editorTeacherFilter').value,dim=filter&&!record.teacherCodes.includes(filter);return `<div class="lessonCard ${selected?'selected ':''}${dim?'filterDimmed':''}" data-record-id="${safe(record.id)}" draggable="${record.locked?'false':'true'}" style="--subject-colour:${safe(s.colour)};--subject-bg:${safe(s.colour)}88"><button class="lockButton" type="button" data-toggle-lock="${safe(record.id)}" title="${record.locked?'Unlock':'Lock'} card">${record.locked?'🔒':'○'}</button><h4>${safe(s.shortName||s.name)}</h4><p>${safe(teacherNames(record.teacherCodes))}</p><p>${safe(venueName(record.venueId))}</p>${record.classIds.length>1?`<div class="combinedFlag">Combined · ${safe(record.classIds.join(' + '))}</div>`:''}</div>`;
}
