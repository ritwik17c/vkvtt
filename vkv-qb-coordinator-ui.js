...
(function(){
  ...
  function filter(){
    const list=document.getElementById('reviewList');if(!list)return;
    const term=(document.getElementById('qbReviewSearch')?.value||'').trim().toLowerCase();let shown=0,total=0,knownAge=0,ageTotal=0,oldest=0;
    const counts={today:0,'0-2':0,'3-7':0,'8plus':0,unknown:0};
    list.querySelectorAll('.qcard').forEach(card=>{total++;const b=card.dataset.qbAgeBucket||bucket(card),a=ageDays(card);counts[b]=(counts[b]||0)+1;if(a!=null){knownAge++;ageTotal+=a;oldest=Math.max(oldest,a)}const textOk=!term||(card.textContent||'').toLowerCase().includes(term),ageOk=selected==='all'||b===selected,ok=textOk&&ageOk;card.style.display=ok?'':'none';if(ok)shown++});
